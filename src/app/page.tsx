...
import { Camera, ChefHat, Download, Heart, Plus, RotateCcw, ShoppingBasket, Sparkles, Star, Upload, Users } from "lucide-react";
...
export default function HomePage() {
  ...
  const suggestionDate = new Date(startOfToday());
  suggestionDate.setDate(suggestionDate.getDate() + 1);
  const urgentTodayItems = [
    ...state.inventory.filter((item) => item.urgent || (item.expiresAt && item.expiresAt <= dateAfter(2))).map((item) => `${item.product}: использовать до ${item.expiresAt ?? "скорее"}`),
    ...state.leftovers.filter((item) => item.useBy <= dateAfter(2)).map((item) => `${item.name}: остатки до ${item.useBy}`),
  ].slice(0, 4);
  ...
  >
    {({ active, navigate }) => <>
          ...
          {active === "Сегодня" && <TodayView meals={todayMeals} shopping={state.shopping} dishMap={dishMap} urgentItems={urgentTodayItems} suggestionDate={suggestionDate.toISOString().slice(0, 10)} slotLabels={slotLabels} onOpenShopping={() => navigate("Покупки")} onReplace={(meal, slot) => setReplaceRequest({ meal, slot })} onRemove={(meal, slot) => commit(removeComponent(state, meal.id, slot), `Убрали ${slotLabels[slot]}.`)} onMove={moveMeal} onRepeat={repeatMeal} onShop={addMealToShopping} onBan={(dish) => commit(banDish(state, dish.id), `${dish.name}: пока не предлагаем.`)} onCook={(meal) => startCooking(meal, navigate)} onQuick={handleQuick} onSuggestFromPantry={(products, minutes, budget) => suggestDishesFromPantry(state, products, minutes, budget)} onPlanSuggested={(dishId, date) => commitMenuResult(planDishForDateResult(state, dishId, date), "Подобранное блюдо поставлено на ужин, покупки пересчитаны.")} />}
          {active === "Меню" && <MenuView meals={state.meals} recipes={state.recipes} dishMap={dishMap} todayIso={today} defaultPlanDate={suggestionDate.toISOString().slice(0, 10)} estimatedCost={estimatedPlanCost(state)} slotLabels={slotLabels} regenerate={regenerate} onReplace={(meal, slot) => setReplaceRequest({ meal, slot })} onPlanMeal={(recipe, kind, date) => {
